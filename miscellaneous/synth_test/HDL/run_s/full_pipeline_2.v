...
`endif
reg [31:0] in3_holding;
reg [31:0] in4_holding;
...
begin
	...
	begin
		...
		`else
			...
			adr_1_op <= shift_1 + in3_holding;
			adr_2_op <= shift_2 + in4_holding;
		`endif
...
`ifdef ADV_ADD
	DW01_add #(width) U4 (.A(shift_1), .B(in3_holding), .CI(1'b0), .SUM(adr_1_op_temp));
	DW01_add #(width) U5 (.A(shift_2), .B(in4_holding), .CI(1'b0), .SUM(adr_2_op_temp));
